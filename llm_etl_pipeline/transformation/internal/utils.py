"""
This module provides functionality for semantically clustering a list of sentences.

It uses Sentence-BERT for generating sentence embeddings and Agglomerative Clustering
to group similar sentences. The primary function `_cluster_list_sents` then
selects the longest sentence from each cluster as its representative, effectively
reducing redundancy in a list of semantically similar sentences.
"""
...
def _cluster_list_sents(
    ...
    distance is below the `threshold`. For each resulting cluster, the longest sentence
    ...
    if sentences and len(sentences) > 1 and model_st and hierarchical_clustering:
        logger.info(
            "Proceeding with clustering as conditions are met "
            "(more than one sentence, models provided)."
        )
        ...
        try:
            ...
            raise ValueError(
                f"Failed to generate sentence embeddings."
                f"Please check the SentenceTransformer model or input data: {e}"
            ) from e
        try:
            ...
            raise ValueError(
                "Failed to perform hierarchical clustering. "
                "Ensure the clustering model is correctly initialized "
                f"and the embeddings are valid: {e}"
            ) from e
        ...
        logger.success(
            f"Clustering complete. Reduced {len(sentences)} sentences "
            f"to {len(result_list)} representative sentences."
        )

    elif sentences and len(sentences) <= 1:
        logger.success(
            "Input list contains 0 or 1 sentence. "
            "Skipping clustering and returning the original list."
        )
    else:
        logger.warning(
            "Clustering skipped: Either input list "
            "or SentenceTransformer model or "
            "AgglomerativeClustering model was not provided."
        )
