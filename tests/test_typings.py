...
class TestCustomTypes:
    ...
    def test_sat_model_id(self):
        ...
        # Mock Path.exists() for the "Valid local path" cases
        with patch("pathlib.Path.exists", return_value=True):
            # Valid local path (string) - NOW MOCKED TO EXIST
            assert MyConfig(
                id=1,
                name="a",
                tags=["a"],
                ...
                ref_depth="paragraphs",
                ext_type="money",
                model_id="/path/to/my/model.bin",
            ).model_id == Path("/path/to/my/model.bin")

            # Valid local path (Path object) - NOW MOCKED TO EXIST
            assert MyConfig(
                id=1,
                name="a",
                tags=["a"],
                ...
                ref_depth="paragraphs",
                ext_type="money",
                model_id=Path("/path/to/another/model.pt"),
            ).model_id == Path("/path/to/another/model.pt")
        ...
        with pytest.raises(
            ValidationError
        ):  # <--- This is likely the "with" statement on line 155
            MyConfig(
                id=1,
                name="a",
                tags=["a"],
